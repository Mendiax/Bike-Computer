...
void View_Max_Avg::render(void)
{
    ...
    creator->reset();
    creator->setup_bar(&this->data.sensors);
    ...
    creator->add_value("%2.0f", 2, &this->data.session.speed.velocityMax, max, Align::CENTER);

    creator->add_value("%4.1f",4,&this->data.session.speed.avg, avg, Align::LEFT);


    creator->add_value("%2d:%02d:%02d", 8, (mtime_t *)&this->data.session.speed.drive_time, time_passed, Align::CENTER);
