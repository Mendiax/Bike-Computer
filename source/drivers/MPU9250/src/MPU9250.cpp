...
// Interrupt data
static Vector3<float> inter_gyro_data;
static Vector3<int16_t> inter_accel_data;
// first read
...
Interrupt interrupt_mpu = {20, mpu_interrupt_callback, GPIO_IRQ_EDGE_RISE};
...
{
  callback_count++;
  mpu9250::read_accel(inter_accel_data);
  mpu9250::read_gyro(inter_gyro_data);
}

std::tuple<Vector3<float>, Vector3<int16_t>> mpu9250::get_mpu_data()
{
  Vector3<float> gyro_data = ::inter_gyro_data;
  Vector3<int16_t> accel_data = ::inter_accel_data;
  // PRINT("mpu callback count => " << callback_count);
  return std::make_tuple(inter_gyro_data, inter_accel_data);
}
