...
void add_millis(const TimeS& time, uint64_t millis)
{
...
{
    auto& gps_time = gps_data.utc_date_time;

    auto millis_to_add = us_to_ms(absolute_time_diff_us(gps_data.data_time_stamp, get_absolute_time()));
    TimeS correct_time;
    correct_time.seconds = ((float)millis_to_add / 1000.0f) + gps_time.seconds;
    ...
    minutes_to_add += gps_time.minutes;
    ...
    correct_time.day = gps_time.day + days_to_add; // TODO

    // TODO month, yaer
    correct_time.month = gps_time.month;
    correct_time.year = gps_time.year;
