	...
	void MPU9250_Init(void)
{
  ...
  consolep("I2C_WriteOneByte() Done\n");
	
	sleep_ms(10);
	if(MPU9250_Check())
	{
  consolep("MPU9250_Check() true\n");
		//GPIO_SetBits(GPIOB, GPIO_Pin_15);
	}
	else
	{
  consolep("MPU9250_Check() false\n");
...
//
  ...
  // }
  accel[0] = (BUF[0]<<8)|BUF[1];//*(OutBuffer + 0);
  accel[1] = (BUF[2]<<8)|BUF[3];//*(OutBuffer + 1);
  accel[2] = (BUF[4]<<8)|BUF[5];//*(OutBuffer + 2); 
}
/**
...
{
  consolep("MPU9250_InitGyrOffset()\n");
