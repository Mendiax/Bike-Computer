...
def struct_to_string(struct_name: str, file_path: str, overwrite : bool) -> str:
    if("::" in struct_name):
        ...
        name = struct_name
    print("Generating", namespace, name)
    ...
    def get_member_tree(members, prefix=""):
        ...
        for field_type, field_name in members:
            if field_type.startswith("struct "):
                ...
                # generate_to_string_function(content, file_path, field_type, tree, overwrite)
                struct_to_string(namespace + "::" + field_type.lstrip("struct "), file_path, overwrite)
    ...
    generate_to_string_function(content, file_path, struct_name, tree, overwrite)

def generate_to_string_function(content : str, file_path : str, struct_name: str, tree: dict, overwrite : bool) -> str:
    function = fr"""#ifndef AUTOGEN_TO_STRING_{struct_name.upper().replace("::", "_")}
static inline std::string to_string(const {struct_name}& obj) {{
    std::stringstream ss;
"""
    function += f"    ss << \"Struct {struct_name}:\\n\";\n"
    INDENT = "    "
