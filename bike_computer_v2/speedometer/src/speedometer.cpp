...
float speed_getSpeed()
{
    ...
    if (us_to_ms(absolute_time_diff_us(speed_lastupdate, update_us)) > MAX_TIME)
    {
        read = true;
        return 0.0;
    }
...
static void speed_update(uint gpio, uint32_t events)
{
    ...
    {
        absolute_time_t update_us = get_absolute_time();
        int64_t delta_time = us_to_ms(absolute_time_diff_us(speed_lastupdate, update_us));
        ...
        DEBUG_SPEED("interrupt : %lu speed : %f delta : %lld last : %lu\n", to_ms_since_boot(update_us), speed_velocity, delta_time,  to_ms_since_boot(speed_lastupdate));
        if (read || 1)
        {
